			...
			for (int posV = 0; posV < (int)params->correlatedVertices[nodeU->cour].size(); posV++)
			{
				...
				if (loopID == 0 || std::max<int>(nodeU->route->whenLastModified, nodeV->route->whenLastModified) > lastTestRINodeU) // only evaluate moves involving routes that have been modified since last move evaluations for nodeU
				{
					// Randomizing the order of the neighborhoods within this loop does not matter much as we are already randomizing the order of the node pairs (and it's not very common to find improving moves of different types for the same node pair)
						...
						// std::cerr << "T_T: " << routeU->cour << " " << routeV->cour << '\n';
...
bool LocalSearch::move1()
{
	...
	double costSuppV = params->timeCost[nodeVIndex][nodeUIndex] + params->timeCost[nodeUIndex][nodeYIndex] - params->timeCost[nodeVIndex][nodeYIndex];
		...
		if (routeU->cour == 1 && routeV->cour == 8) {
			std::cerr << "HELP ME : " << costSuppU << " " << costSuppV << '\n';
		}
...
LocalSearch::LocalSearch(Params * params) : params (params)
{
	...
	newNext = std::vector < Node* > (params->nbClients + 1);
...
double LocalSearch::getTimeCost(Node *u, Node*v) 
{
	return params -> timeCost[u->cour][v->cour];
}

double LocalSearch::routeTimeCost(Route *route) 
{
	double result = getTimeCost(route->depot, route->depot->next);
	for (Node *u = route->depot->next; !u->isDepot; u = u->next) {
		result += getTimeCost(u, u->next);
	}
	return result;
}

double LocalSearch::routeDemand(Route *route) 
{
	int result = 0;
	for (Node *u = route->depot->next; !u->isDepot; u = u->next) {
		result += params->cli[u->cour].demand;
	}
	return result;
}

void LocalSearch::debugRoute(Route *route, bool assertActivation) 
{
	std::cerr << "Debug Route: (" << route->cour << ", " << routeDemand(route) << ", " << routeTimeCost(route) << "): ";	
	
	int numCustomers = 0;
	std::vector<int> V;
	for (Node *u = route->depot; ; u = u->next) {
		std::cerr << u->cour << "(" << params->cli[u->cour].demand << ", " << u->route->cour <<  ") ";
		V.push_back(u->cour);
		if (u == route->depot) {
			continue;
		}
		...
		++numCustomers;
		if (numCustomers > route->nbCustomers) {
			std::cerr << "\nBUG IN NUMBER OF CUSTOMERS!!!!!!\n";
			assert(0 == 1);
		}
	...
	// std::cerr << "Debug reverse route: ";
	for (Node* u = route->depot->prev; ; u = u -> prev) {
		// std::cerr << u->cour << "(" << params->cli[u->cour].demand << ", " << u->route->cour <<  ") ";
		if (u->cour != V.back()) {
			std::cerr << "BUG IN REVERSE LINK\n";
			exit(0);
		}
		V.pop_back();
		...
		if (assertActivation) {
			assert(route->depot->route == route);
			for (Node *u = route->depot; ; u = u->next) {
				assert(u->route->cour == route->cour);
				if (u == route->depot) {
					continue;
				}
				if (u->isDepot) {
...
bool LocalSearch::swapDoubleStar() {
	...
	std::vector<std::vector<std::vector<SwapDoubleStarElement> > > 
		...
		std::vector<std::vector<SwapDoubleStarElement>>(routeV->nbCustomers+2, 
		std::vector<SwapDoubleStarElement> (3, {INFINITY, -1, -1, INFINITY})));
	std::vector<std::vector<std::vector<SwapDoubleStarTraceElement> > > 
		Trace(routeU->nbCustomers+2, 
		std::vector<std::vector<SwapDoubleStarTraceElement>>(routeV->nbCustomers+2, 
		std::vector<SwapDoubleStarTraceElement> (3, {-2, -2, -2})));		

	// dp 
	dp[0][0][0] = { 0, 0, 0, 0 };
	Trace[0][0][0] = { -1, -1, -1 };
	
	int cumulatedLoadU = 0;
	for (int i = 0; i <= routeU->nbCustomers+1; ++i) 
	{
		nodeU = (i == 0) ? routeU->depot : nodeU -> next;
		cumulatedLoadU += params->cli[nodeU->cour].demand;

		int cumulatedLoadV = 0;
		for (int j = 0; j <= routeV->nbCustomers+1; ++j) 
		{
			nodeV = (j == 0) ? routeV -> depot : nodeV -> next;
			cumulatedLoadV += params->cli[nodeV->cour].demand; 

			if (Trace[i][j][2].type != -2) 
			{
				SwapDoubleStarElement value = dp[i][j][2];

				if (i+1 <= routeU->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta;
					newValue.demandU = value.demandU + params->cli[nodeU->next->cour].demand;
					newValue.demandV = value.demandV;
					newValue.fakeDeltaCost = value.fakeDeltaCost;

					if (dp[i+1][j][2].demandU == -1 || dp[i+1][j][2] > newValue) {
						dp[i+1][j][2] = newValue;
						Trace[i+1][j][2] = { i, j, 2 };
					}
				}

				if (i+2 <= routeU->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeV->prev->prev, nodeU->next) + getTimeCost(nodeU->next, nodeV)
												 - getTimeCost(nodeU, nodeU->next) - getTimeCost(nodeU->next, nodeU->next->next);
					newValue.demandU = value.demandU + params->cli[nodeU->next->next->cour].demand;
					newValue.demandV = value.demandV + params->cli[nodeU->next->cour].demand;
					newValue.fakeDeltaCost = newValue.delta
									+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU - params->cli[nodeU->next->cour].demand - params->cli[nodeU->next->next->cour].demand)
									+ penaltyExcessLoad(newValue.demandV + routeV->load - cumulatedLoadV)
									- routeU->penalty - routeV->penalty;

					if (dp[i+2][j][1].demandU == -1 || dp[i+2][j][1] > newValue) {
						dp[i+2][j][1] = newValue;
						Trace[i+2][j][1] = { i, j, 2 };
					}
				}
				...
				if (0 != 1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeV->prev->prev, nodeV);
					newValue.demandU = value.demandU;
					newValue.demandV = value.demandV;
					newValue.fakeDeltaCost = newValue.delta
											+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU)
											+ penaltyExcessLoad(newValue.demandV + routeV->load - cumulatedLoadV)
											- routeU->penalty - routeV->penalty;
					assert( abs(newValue.fakeDeltaCost - (value.fakeDeltaCost + newValue.delta - value.delta)) <= MY_EPSILON );

					if (dp[i][j][0].demandU == -1 || dp[i][j][0] > newValue) {
						dp[i][j][0] = newValue;
						Trace[i][j][0] = { i, j, 2 };
					}
				}
			...
			if (Trace[i][j][1].type != -2) 
			{
				SwapDoubleStarElement value = dp[i][j][1];

				if (j+1 <= routeV->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta;
					newValue.demandU = value.demandU;
					newValue.demandV = value.demandV + params->cli[nodeV->next->cour].demand;
					newValue.fakeDeltaCost = value.fakeDeltaCost;

					if (dp[i][j+1][1].demandU == -1 || dp[i][j+1][1] > newValue) {
						dp[i][j+1][1] = newValue;
						Trace[i][j+1][1] = { i, j, 1 };
					}
				}

				if (j+2 <= routeV->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeU->prev->prev, nodeV->next) + getTimeCost(nodeV->next, nodeU)
												 - getTimeCost(nodeV, nodeV->next) - getTimeCost(nodeV->next, nodeV->next->next);
					newValue.demandU = value.demandU + params->cli[nodeV->next->cour].demand;
					newValue.demandV = value.demandV + params->cli[nodeV->next->next->cour].demand;
					newValue.fakeDeltaCost = newValue.delta
									+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU)
									+ penaltyExcessLoad(newValue.demandV + routeV->load - cumulatedLoadV - params->cli[nodeV->next->cour].demand - params->cli[nodeV->next->next->cour].demand) 
									- routeU->penalty - routeV->penalty;

					if (dp[i][j+2][2].demandU == -1 || dp[i][j+2][2] > newValue) {
						dp[i][j+2][2] = newValue;
						Trace[i][j+2][2] = { i, j, 1 };
					}
				}
				...
				if (0 != 1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeU->prev->prev, nodeU);
					newValue.demandU = value.demandU;
					newValue.demandV = value.demandV;
					newValue.fakeDeltaCost = newValue.delta
									+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU)
			...
			if (Trace[i][j][0].type != -2) 
			{
				SwapDoubleStarElement value = dp[i][j][0];
				...
				if (i+1 <= routeU->nbCustomers+1) {
					...
					if (dp[i+1][j][0].demandU == -1 || dp[i+1][j][0] > newValue) {
						dp[i+1][j][0] = newValue;
						Trace[i+1][j][0] = { i, j, 0 };
					}
				}
				...
				if (j+1 <= routeV->nbCustomers+1) {
					...
					if (dp[i][j+1][0].demandU == -1 || dp[i][j+1][0] > newValue) {
						dp[i][j+1][0] = newValue;
						Trace[i][j+1][0] = { i, j, 0 };
					}
				}

				if (i+2 <= routeU->nbCustomers+1 && j+1 <= routeV->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeV, nodeU->next) + getTimeCost(nodeU->next, nodeV->next) 
												 - getTimeCost(nodeV, nodeV->next)
												 - getTimeCost(nodeU, nodeU->next) - getTimeCost(nodeU->next, nodeU->next->next);
					newValue.demandU = value.demandU + params->cli[nodeU->next->next->cour].demand;
					newValue.demandV = value.demandV + params->cli[nodeU->next->cour].demand + params->cli[nodeV->next->cour].demand; 
					newValue.fakeDeltaCost = newValue.delta 
									+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU - params->cli[nodeU->next->cour].demand - params->cli[nodeU->next->next->cour].demand)
									+ penaltyExcessLoad(newValue.demandV + routeV->load - cumulatedLoadV - params->cli[nodeV->next->cour].demand)
									- routeU->penalty - routeV->penalty;

					if (dp[i+2][j+1][1].demandU == -1 || dp[i+2][j+1][1] > newValue) {
						dp[i+2][j+1][1] = newValue;
						Trace[i+2][j+1][1] = { i, j, 0 };
					}
				}
				if (i+1 <= routeU->nbCustomers+1 && j+2 <= routeV->nbCustomers+1) {
					SwapDoubleStarElement newValue;
					newValue.delta = value.delta + getTimeCost(nodeU, nodeV->next) + getTimeCost(nodeV->next, nodeU->next)
												 - getTimeCost(nodeU, nodeU->next)
												 - getTimeCost(nodeV, nodeV->next) - getTimeCost(nodeV->next, nodeV->next->next);
					newValue.demandU = value.demandU + params->cli[nodeV->next->cour].demand + params->cli[nodeU->next->cour].demand;
					newValue.demandV = value.demandV + params->cli[nodeV->next->next->cour].demand;
					newValue.fakeDeltaCost = newValue.delta
									+ penaltyExcessLoad(newValue.demandU + routeU->load - cumulatedLoadU - params->cli[nodeU->next->cour].demand)
									+ penaltyExcessLoad(newValue.demandV + routeV->load - cumulatedLoadV - params->cli[nodeV->next->cour].demand - params->cli[nodeV->next->next->cour].demand)
									- routeU->penalty - routeV->penalty;

					if (dp[i+1][j+2][2].demandU == -1 || dp[i+1][j+2][2] > newValue) {
						dp[i+1][j+2][2] = newValue;
						Trace[i+1][j+2][2] = { i, j, 0 };
					}
				}
			}
		}
	...
	int n = routeU->nbCustomers+1, oldN = n;
	int m = routeV->nbCustomers+1, oldM = m;
	int type = 0;
	...
	while (n > 0 || m > 0) {
		...
		if (prev.type == 0) 
		{
			if (type == 0) {
				...
				assert(n == prev.n + 2 && m == prev.m + 1);
				nodeX = nodeU->prev->prev;
				nodeY = nodeV->prev;

				// newNext[nodeV->prev->cour] = nodeU->prev;
				// newNext[nodeU->prev->cour] = nodeV;
				nodeV->prev->next = nodeU->prev;
				nodeU->prev->next = nodeV;
			} 
			else if (type == 2) {
				assert(n == prev.n + 1 && m == prev.m + 2);
				nodeX = nodeU->prev;
				nodeY = nodeV->prev->prev;

				// newNext[nodeU->prev->cour] = nodeV->prev;
				// newNext[nodeV->prev->cour] = nodeU;
				nodeU->prev->next = nodeV->prev;
				nodeV->prev->next = nodeU;
			}
		} 
		else if (prev.type == 1) 
		{
			if (type == 0) {
				assert(n == prev.n && m == prev.m);
				nodeX = nodeU;
				nodeY = nodeV;

				// newNext[nodeU->prev->prev->cour] = nodeU;
				nodeU->prev->prev->next = nodeU;
			} 
			else if (type == 1) {
				assert(m == prev.m + 1);
				nodeX = nodeU;
				...
				// newNext[nodeU->prev->prev->cour] = nodeV->prev;
				// newNext[nodeV->prev->cour] = nodeU; 
				nodeU->prev->prev->next = nodeV->prev;
				nodeV->prev->next = nodeU; 
			}
		}
		else if (prev.type == 2) 
		{
			if (type == 0) {
				...
				// newNext[nodeV->prev->prev->cour] = nodeV;
				nodeV->prev->prev->next = nodeV;
			} 
			else if (type == 1) {
				assert(n == prev.n + 2);
				nodeX = nodeU->prev->prev;
				...
				// newNext[nodeV->prev->prev->cour] = nodeU->prev;
				// newNext[nodeU->prev->cour] = nodeV;
				nodeV->prev->prev->next = nodeU->prev;
				nodeU->prev->next = nodeV;
			}
			else if (type == 2) {
				assert(n == prev.n + 1);
	...
	nodeU = routeU->depot;
	while (true) {
		nodeU->next->prev = nodeU;
		nodeU = nodeU->next;
		if (nodeU->isDepot) break;
	}

	nodeV = routeV->depot;
	while(true) {
		nodeV->next->prev = nodeV;
		nodeV = nodeV->next;
		if (nodeV->isDepot) break;
	}
	...
	updateRouteData(routeV);
		...
		double newCost = routeTimeCost(routeU) + routeTimeCost(routeV);
		double newScore = newCost + routeU->penalty + routeV->penalty;
		assert( abs(newCost - oldCost - dp[oldN][oldM][0].delta) < MY_EPSILON );
		assert( abs(newScore - oldScore - dp[oldN][oldM][0].fakeDeltaCost) < MY_EPSILON );
		assert( routeU->load == dp[oldN][oldM][0].demandU );
		assert( routeV->load == dp[oldN][oldM][0].demandV );
